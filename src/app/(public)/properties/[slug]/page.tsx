...
import { ButtonLink, getPropertyPageVisual, PropertyPhoto, PublicPageHero } from '@/components/PublicUI';
...
export async function generateMetadata(props: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const visual = getPropertyPageVisual(property.slug, property.serviceType);
  return { title: `${property.publicName} - ${property.neighbourhood}`, description: property.publicSummary, openGraph: { images: [{ url: visual.src, alt: visual.alt }] } };
}
...
export default async function PropertyDetailPage(props: { params: Promise<{ slug: string }> }) {
  ...
  const featuredImage = getPropertyPageVisual(property.slug, property.serviceType);

  return <main>
    <PublicPageHero eyebrow={property.serviceType} title={`${property.publicName} - ${property.neighbourhood}`} intro={property.publicSummary} image={featuredImage} />
